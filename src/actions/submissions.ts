...
export async function gradeCode(
  ...
  if (!user) {
    return { error: "Unauthorized", httpStatus: 401, status: "", score: 0, passed: 0, total: 0, results: [] };
  }
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : "Judge0 unavailable";
    const isNotFound   = (err as { code?: string }).code === "NOT_FOUND";
    const isNoTestCase = (err as { code?: string }).code === "NO_TEST_CASES";

    if (isNotFound)   return { error: "Problem not found",          httpStatus: 404, status: "", score: 0, passed: 0, total: 0, results: [] };
    if (isNoTestCase) return { error: "No test cases for problem",  httpStatus: 400, status: "", score: 0, passed: 0, total: 0, results: [] };

    // Judge0 is offline — record the attempt before returning the error.
    await recordOfflineSubmission(
      { userId: user.id, problemId, code, languageId },
      count ?? 0
    );

    return { error: message, httpStatus: 503, status: "JUDGE0_OFFLINE", score: 0, passed: 0, total: 0, results: [] };
  }
}
